		...
		},
{
 ...
 let  owners = ['1258957407258673254']
		const user = interaction.options.getMember('user')
    const input = interaction.options.getString('input')
          if(!owners.includes(interaction.user.id)) return await interaction.editReply("You don't have permission 🙄")
      ...
      if(user.user.bot) return interaction.editReply({content :`You can't use this command with bot.`});
        ...
        await db.add(`money_${user.id}`, input)
        let bal = await await db.get(`money_${user.id}`)
